        ...
        if self.isTraining:
            if duration-snip_duration <= 0:
                ...
                if np.random.random()>0.2:
                    ...
                    for i in range(self.num_splits):
                        start_index = np.random.randint(0,snip_duration//10)
                        indices = list(range(start_index,duration-snip_duration,snip_duration))
                        if len(indices)==0:
                            indices = list(range(0,duration-snip_duration+1,snip_duration))
            ...
            if np.random.random()>0.5:
                for j in range(len(y_snippets)):
                    y_new = np.zeros(self.conf.sampling_rate*self.conf.duration)
                    mixture_num = np.random.randint(1,5)
                    for i in range(5):
                        temp = []
                        for ci in np.random.choice(self.selection_index,mixture_num, replace=False):
                            ci_y = y_snippets[j][ci:ci+self.conf.sampling_rate]
                            temp.append(ci_y)
                        temp = np.array(temp).mean(axis=0)
                        y_new[i*self.conf.sampling_rate:i*self.conf.sampling_rate+self.conf.sampling_rate] = temp
                    y_snippets[j] = y_new
